...
function P1card({ database }) {
  // Build full image URL; fallback to placeholder if missing
  const imageUrl = database?.image
    ? database.image.startsWith("http")
      ? database.image
      : API_BASE + database.image
    : "/placeholder.png"; // add a real placeholder image in /public
  ...
  return (
    <div className="card border-0 shadow-sm col-3 text-start">
      ...
      <div className="card-body">
        ...
        <p className="fs-3">{database?.name}</p>

        <div className="mt-2">
          <span className="text-secondary text-decoration-line-through me-2">
            $360
          </span>
          <span className="fw-bold text-primary">{database?.price}</span>
        </div>
